...
This manual explains how to send custom **Z tables** through OneConnect by combining the SAP Data Modeler's **ANY entity** with a custom **Z program** that reacts to create, update, and delete events on those tables.
...
The example below sends a sales order structure composed of a **HEADER** table (`ZONTA_SO_H`) and an **ITEMS** table (`ZONTA_SO_ITEM`).
...
| **`iv_dest`** | RFC destination defined in transaction `SM59`. |
| **`iv_bothnames`** | Set to `abap_true` to send both technical name and alias concatenated. |

> ℹ️ **Note:** When sending a create or update operation, pass `iv_update = abap_true` and `iv_delete = abap_false`. For delete operations, pass `iv_update = abap_false` and `iv_delete = abap_true`. The `iv_entity_business_proc` parameter must always be set to the name of your entity.
